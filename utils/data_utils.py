...
def parse_data(line, class_num, img_size, anchors, mode):
    ...
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB) #comment for grayscale images
...
def double_parse_data(line, class_num, img_size, anchors, mode):
    '''
    param:
        line: a line from the training/test txt file
        args: args returned from the main program
        mode: 'train' or 'val'. When set to 'train', data_augmentation will be applied.
    '''
    pic_path_ir, boxes, labels = parse_line(line)
    pic_path_co = list(pic_path_ir)
    pic_path_co_prefix = ''.join(pic_path_co[:-21])
    pic_path_co_suffix1 = ''.join(pic_path_co[-19:-6])
    pic_path_co_suffix2 = ''.join(pic_path_co[-4:])
    pic_path_co = pic_path_co_prefix + 'co' + pic_path_co_suffix1 + 'co' + pic_path_co_suffix2

    img_ir = cv2.imread(pic_path_ir)
    img_co = cv2.imread(pic_path_co)
    img_co = cv2.cvtColor(img_co, cv2.COLOR_BGR2RGB) #comment for grayscale images

    img_ir, boxes = resize_image_and_correct_boxes(img_ir, boxes, img_size)
    img_co, _ = resize_image_and_correct_boxes(img_co, boxes, img_size)

    # do data augmentation here
    if mode == 'train':
        img_ir, boxes, labels = data_augmentation(img_ir, boxes, labels)
        img_co, boxes, labels = data_augmentation(img_co, boxes, labels)
    ...
    return img_ir, img_co, y_true_13, y_true_26, y_true_52
