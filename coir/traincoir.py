...
import sys
sys.path[:0] = ['../']
from utils.data_utils import double_parse_data
...
parser.add_argument("--total_epoches", type=int, default=2500,
...
train_dataset = train_dataset.apply(tf.contrib.data.map_and_batch(
    lambda x: tf.py_func(double_parse_data, [x, flag.class_num, flag.img_size, flag.anchors, 'train'], [tf.float32, tf.float32, tf.float32, tf.float32, tf.float32]),
...
val_dataset = val_dataset.apply(tf.contrib.data.map_and_batch(
    lambda x: tf.py_func(double_parse_data, [x, flag.class_num, flag.img_size, flag.anchors, 'val'], [tf.float32, tf.float32, tf.float32, tf.float32, tf.float32]),
    batch_size=flag.batch_size))
...
image_ir, image_co, y_true_13, y_true_26, y_true_52 = dataset_iterator.get_next()
y_true = [y_true_13, y_true_26, y_true_52]

# tf.data pipeline will lose the data shape, so we need to set it manually
image_ir.set_shape([None, flag.img_size[1], flag.img_size[0], 3])
image_co.set_shape([None, flag.img_size[1], flag.img_size[0], 3])
for y in y_true:
...
with tf.variable_scope('yolov3'):
    pred_feature_maps = yolo_model.forward(image_ir, image_co, is_training=is_training)
...
saver_to_restore = tf.train.Saver(var_list=tf.contrib.framework.get_variables_to_restore(include=['yolov3/darknet53_body']))
saver_to_restore_co = tf.train.Saver(var_list=tf.contrib.framework.get_variables_to_restore(include=['yolov3/darknet53_body_co']))
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    saver_to_restore_co.restore(sess, flag.restore_path)
