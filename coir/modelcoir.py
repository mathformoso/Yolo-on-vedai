...
class yolov3(object):
    ...
    def forward(self, input_ir, input_co, is_training=False, reuse=False):
        # the input img_size, form: [height, weight]
        # it will be used later
        self.img_size = tf.shape(input_ir)[1:3]
        ...
        with slim.arg_scope([slim.conv2d, slim.batch_norm],reuse=reuse):
            with slim.arg_scope([slim.conv2d], normalizer_fn=slim.batch_norm,
                ...
                with tf.variable_scope('darknet53_body_ir'):
                    route_1ir, route_2ir, route_3ir = darknet53_body(input_ir)
                with tf.variable_scope('darknet53_body_co'):
                    route_1co, route_2co, route_3co = darknet53_body(input_co)

                route_1 = tf.concat([route_1ir, route_1co], axis=3)
                route_2 = tf.concat([route_2ir, route_2co], axis=3)
                route_3 = tf.concat([route_3ir, route_3co], axis=3)
